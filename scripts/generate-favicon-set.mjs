...
// Heart bounding box inside logo.svg (2x render of 1024×640 viewBox).
const HEART = { left: 707, top: 299, width: 593, height: 535 };

/**
 * Extract the red heart from og-image.png with white → transparent.
 */
...
/**
 * Place the heart centred on an N×N canvas. bg = null → transparent.
 */
...
/**
 * Build icon.svg: heart-only viewBox + brand left-pink / right-magenta split.
 */
...
/**
 * to-ico@1.0.1 mislabels directory width/height as 0 (meaning 256). Patch
 * from each image's real DIB header so crawlers see correct sizes.
 */
function fixIcoSizes(ico) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const height = ico.readInt32LE(dibOffset + 8) / 2;
