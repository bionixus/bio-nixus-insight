#!/usr/bin/env node
/**
 * BIO-439: Apply LLM/GEO optimization to BIO-435 Day 1 blog batch.
 *
 * Usage:
 *   node scripts/geo/apply-bio435-day1-geo-pass.mjs --dry-run
 *   node scripts/geo/apply-bio435-day1-geo-pass.mjs --apply
 *   node scripts/geo/apply-bio435-day1-geo-pass.mjs --apply --sync-sanity
 */
...
import { GEO_PASS_BY_SLUG, buildGeoAnswerHtml } from '../lib/bio435-geo-pass-config.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const root = path.join(__dirname, '../..');
const dataDir = path.join(root, 'scripts/data');
const reportPath = path.join(root, 'docs/geo/bio-439-day1-geo-pass-2026-06-18.md');

const DAY1_SLUGS = [
  'sfda-economic-evaluation-budget-impact-guide-2026',
  ...JSON.parse(fs.readFileSync(path.join(dataDir, 'bio435-day1-manifest.json'), 'utf8')).articles.map(
    (a) => a.slug,
  ),
];
...
function buildReport(results) {
  const lines = [
    ...
    '## Summary',
    '',
    `Applied LLM/GEO optimization to **${results.filter((r) => r.applied).length}/${results.length}** Day-1 blog URLs per [geo-entity-playbook.md](../geo-entity-playbook.md).`,
    '',
    '### Lenses applied',
    '',
  ...
  for (const r of results) {
    lines.push(
      `| \`${r.slug}\` | ${r.after.entitySentence ? '✅' : '❌'} | ${r.after.geoBlock ? '✅' : '❌'} | ${r.after.faqCount} | ${r.after.hubLink ? '✅' : '⚠️'} | ${r.sanitySync || '—'} |`,
    );
  }
  ...
  lines.push(
    ...
    '| Medium | SEOSpecialist verify Article schema + sitemap includes new URLs | SEOSpecialist |',
...
async function main() {
  ...
  for (const slug of slugs) {
    const config = GEO_PASS_BY_SLUG[slug];
    if (!config) {
