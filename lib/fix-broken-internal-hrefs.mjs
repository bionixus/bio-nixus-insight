...
import { resolveGlobalWebsitesRedirect } from './global-websites-redirects.mjs';
...
export const HREF_REWRITES = {
  ...
  '/pharmacoeconomics-gcc': '/blog/pharmacoeconomics-gcc-practical-guide',
...
/**
 * Resolve a path-only internal URL that 301s elsewhere (or null if already canonical).
 * @param {string} pathOnly
 * @returns {string | null}
 */
export function resolveRedirectHrefPath(pathOnly) {
  if (!pathOnly || typeof pathOnly !== 'string') return null;
  let path = pathOnly.split(/[?#]/)[0] || pathOnly;
  if (path.length > 1 && path.endsWith('/')) path = path.replace(/\/+$/, '');

  // Try decoded + encoded keys (Arabic blog aliases appear both ways in crawls).
  ...
  for (const key of candidates) {
    if (HREF_REWRITES[key]) {
      const dest = HREF_REWRITES[key];
      // Decode Location-style encodeURI targets for in-page hrefs when safe.
      try {
  ...
  const gw = resolveGlobalWebsitesRedirect(path);
  return gw || null;
}
...
export function fixBrokenInternalHref(href) {
  ...
  if (/^https?:\/\//i.test(raw)) {
    ...
    if (m) {
      ...
      const resolved = resolveRedirectHrefPath(pathOnly);
      if (resolved) return `${origin}${resolved}${suffix}`;
