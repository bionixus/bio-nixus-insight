/**
 * Crawler HTML for /news/:slug (invoked from api/blog/[slug].js via ?pressRelease=1).
 */
...
export const PRESS_CRAWLER_QUERY = `*[_type == "pressRelease" && slug.current == $slug][0]{
  headline,
  subheadline,
  dateline,
  body,
...
export async function handlePressReleaseCrawler(req, res, sanityClient, slug) {
  if (!slug || slug === 'feed.xml') {
    return res.status(404).send('Not found');
  }
  try {
    const post = await sanityClient.fetch(PRESS_CRAWLER_QUERY, { slug });
    ...
    console.error('press crawler error', err);
