...
/**
 * Method/topic slugs that are not countries but match the LEGACY_PATH regex
 * shape below (e.g. /quantitative-healthcare-market-research). Excluded so
 * they aren't misread as `/{country}-healthcare-market-research` — doing so
 * created a real redirect loop in production: this function would redirect
 * /quantitative-healthcare-market-research to
 * /healthcare-market-research/quantitative, whose own CountryPage.tsx
 * NON_COUNTRY_SLUG_REDIRECTS safety net (it doesn't recognize "quantitative"
 * as a real country either) redirects straight back. Keep in sync with that
 * map in src/pages/healthcare-research/CountryPage.tsx.
 */
const NON_COUNTRY_SLUGS = new Set(['quantitative', 'qualitative']);
...
/** Countries with live `/{slug}-medtech-market-research` BOFU pages — keep in sync with src/data/developedMarketMedtechPages.ts DEVELOPED_MARKET_MEDTECH_SLUGS. */
...
/**
 * Real pages whose URLs happen to match the legacy `{country}-{industry}-market-research`
 * shape. Redirecting these sends live pages away, and `/quantitative-healthcare-market-research`
 * additionally formed an infinite loop with the `/healthcare-market-research/quantitative`
 * redirect in vercel.json.
 *
 * This is an explicit list because the module runs in the Vercel Edge runtime and cannot
 * import the React router. Add an entry whenever a route of this shape gets its own page.
 */
const DEDICATED_PAGE_PATHS = new Set([
...
export function resolveLegacyCountryIndustryMarketResearchRedirect(pathname) {
  if (DEDICATED_PAGE_PATHS.has(pathname)) return null;
  ...
  if (NON_COUNTRY_SLUGS.has(countrySlug)) return null;

  // Dedicated MedTech BOFU pages — serve at canonical path, do not redirect to pharma.
